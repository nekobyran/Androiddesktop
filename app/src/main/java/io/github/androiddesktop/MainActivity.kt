...
class MainActivity : Activity() {
    ...
    private fun copyAdbCommands() {
        ...
        clipboard.setPrimaryClip(ClipData.newPlainText("Androiddesktop ADB commands", commands))
    ...
    private fun copyWindowingCommands() {
        ...
        val clipboard = getSystemService(Context.CLIPBOARD_SERVICE) as ClipboardManager
        clipboard.setPrimaryClip(ClipData.newPlainText("Androiddesktop windowing commands", commands))
        output.text = commands
        Toast.makeText(this, "窗口化 ADB 命令已复制", Toast.LENGTH_SHORT).show()
    }

    private fun showHuaweiProbe() {
        val snapshot = desktopModeController.collectSnapshot(this)
        output.text = huaweiVisualProbe.buildProbePlan(snapshot)
    }

    private fun showPrinciplePlan() {
        output.text = privilegedCommandPlanner.explain()
    }
